class PronunciationTextsController < ApplicationController
  def index
    ...
    gon.reference_text = @reference_text.map do |text|
      { id: text.id, english: text.english_text, japanese: text.japanese_text, difficulty: text.difficulty }
    end
    gon.reference_text_hard = @reference_text_hard.map do |text|
      { id: text.id, english: text.english_text, japanese: text.japanese_text, difficulty: text.difficulty }
    end
  end
end
